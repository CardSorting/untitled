        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('stickers.people.store') }}">
                        @csrf

                        <div>
                            <x-input-label for="person_name" :value="__('Person Name')" />
                            <x-text-input id="person_name" class="block mt-1 w-full" type="text" name="person_name" :value="old('person_name')" required autofocus />
                            <x-input-error :messages="$errors->get('person_name')" class="mt-2" />
                        </div>
