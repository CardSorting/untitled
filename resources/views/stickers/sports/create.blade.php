        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('stickers.sports.store') }}">
                        @csrf

                        <div>
                            <x-input-label for="sports_type" :value="__('Sports Type')" />
                            <x-text-input id="sports_type" class="block mt-1 w-full" type="text" name="sports_type" :value="old('sports_type')" required autofocus />
                            <x-input-error :messages="$errors->get('sports_type')" class="mt-2" />
                        </div>
